...
// Note: Use 127.0.0.1 instead of localhost to force TCP/IP connection (not Unix socket)
...
function getDB() {
    ...
    if ($conn === null) {
        try {
            // Force TCP/IP by ensuring port is passed and using numeric IP if available
            $host = DB_HOST;
            $port = DB_PORT;
            
            // If host is "localhost", change to 127.0.0.1 to force TCP/IP (not Unix socket)
            if ($host === 'localhost') {
                $host = '127.0.0.1';
            }
            ...
            if ($conn->connect_error) {
                throw new Exception("Connection failed: " . $conn->connect_error . " (Host: $host:$port, User: " . DB_USER . ", DB: " . DB_NAME . ")");
            }
